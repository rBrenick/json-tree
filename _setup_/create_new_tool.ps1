...
# renames all the instances of the current JsonTree with the prompted new name
# searches through files and replaces in there as well

# parent directory of the json-tree
$TOOLS_DIR = (Get-Item -Path "..\").FullName

# define current tool name variables
$CURRENT_base_case = "json-tree"
$CURRENT_snake_case = "json_tree"
$CURRENT_PascalCase = "JsonTree"
...
# Find instances of json_tree in files and replace them with the new tool name
...
Foreach-Object {
    ...
    ((Get-Content -path $_.FullName -Raw) -replace "json-tree", $NEW_base_case) | Set-Content -Path $_.FullName
