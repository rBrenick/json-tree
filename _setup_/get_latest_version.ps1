...
$TOOL_GIT_REPO = "https://github.com/rBrenick/json-tree/archive/main.zip" # modify these after uploading the tool
$UPDATE_PATHS = @("docs", "json_tree", "README.md", "setup.py")

$CURRENT_TOOL_FOLDER = (Get-Item -Path ".").FullName
$CURRENT_json_tree = (Get-Item -Path ".").Name

$NEW_VERSION_ZIP = $USER_DOWNLOADS_FOLDER + "\" + $CURRENT_json_tree + "_update.zip"
$NEW_VERSION_ZIP_FOLDER = $USER_DOWNLOADS_FOLDER + "\" + $CURRENT_json_tree + "_update"
...
foreach ($element in $UPDATE_PATHS) {
    ...
    $src_file_path = "$NEW_VERSION_ZIP_FOLDER\$CURRENT_json_tree-master\$element"
